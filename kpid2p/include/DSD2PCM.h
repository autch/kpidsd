...
public:
	...
	long writeFinal(std::vector<float> resample_data, size_t odone, uint8_t* out)
	{
		uint8_t* op = out;
	...
	long Render(uint8_t* src, size_t src_size, size_t block_size, int lsbf, uint8_t* dst, size_t samplesToRender)
	{
		...
		soxr_error_t e = soxr_process(soxr, float_data.data(), src_size / channels, &idone, resample_data.data(), src_size / channels, &odone);
		if (odone > 0) {
	...
	long RenderLast()
	{
		...
		size_t idone = 0, odone = 0;
		do {
			soxr_error_t e = soxr_process(soxr, NULL, 0, &idone, resample_data.data(), src_size / channels, &odone);
			if (odone > 0) {
				buffer.insert(buffer.end(), resample_data.begin(), resample_data.begin() + odone * channels);
				buffer_stored += odone;
			}
	...
	long RenderFlush(uint8_t* dst, size_t samplesToRender)
	{
		if (buffer_stored == 0) {
			return 0;
		}
